...
class ViewController: UIViewController {
    ...
    @IBAction func clickButton(_ sender: Any) {
        if accountText.text == myAccount && passwordText.text == myPassword {
            ...
            alert.addAction(action)
            present(alert, animated:  true)
        }else if passwordText.state.isEmpty == false && accountText.state.isEmpty == false && accountText.text != myAccount && passwordText.text != myPassword {
            let alert = UIAlertController(title: "Error", message: "Login fail.", preferredStyle: .alert)
            let action = UIAlertAction(title: "OK", style: .default, handler: { action in
            })
            alert.addAction(action)
            present(alert, animated:  true)
        }else if checkText.state.isEmpty == true && accountText.state.isEmpty == false && passwordText.state.isEmpty == false {
